...
`Facade`ベースの””class””では、`__callStatic()`マジックメソッドを使って、あなたの””facade””からコンテナから解決された””object””への呼び出しを遅延させます。以下の例では、””Laravel cache””システムへの呼び出しが行われています。この””コード””を一見すると、静的な`get` ””method””が`Cache` ””class””上で呼び出されていると思うかもしれません:
