...
> [!WARNING]  
> `shouldBeSearchable` method は、Scout の Database エンジンを使用する際には適用できません。なぜなら、すべての検索可能な data は常に database に保存されているからです。同様の振る舞いを Database エンジンを使用して達成するためには、代わりに[where 節](#where-clauses)を使用すべきです。
