...
ただし、ローカル開発中など、Reverb サーバーが直接 handle 安全な接続を処理することが有用な場合もあります。あなたが[Laravel Herd's](https://herd.laravel.com) 安全なサイト feature を使用しているか、[Laravel Valet](/ja/php/laravel/v11.x/10-packages/valet) を使用していて、あなたの application に対して[secure command](/docs/11.x/valet#securing-sites) を実行した場合、あなたのサイトのために生成された Herd / Valet の証明書を使用して Reverb 接続を保護することができます。そのためには、`REVERB_HOST` environment 変数をサイトのホスト名に設定するか、Reverb サーバーを開始する際に明示的にホスト名オプションを渡します。
