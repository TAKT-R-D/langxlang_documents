...
この作業は、`Folio::path`および`Folio::uri`メソッドを使用して達成することができます。`path` method は、routing HTTP requests の際に Folio がページをスキャンするディレクトリを登録し、`uri` method はそのページディレクトリの「ベース URI」を指定します：
