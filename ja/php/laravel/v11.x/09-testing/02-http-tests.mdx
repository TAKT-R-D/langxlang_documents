...
> [!NOTE]
> `assertJson`の method は、 response を array に変換し、`PHPUnit::assertArraySubset`を利用して、指定された array が application が返す JSON response 内に存在することを確認します。そのため、 JSON response に他のプロパティがあっても、指定された fragment が存在していれば、このテストはパスします。
