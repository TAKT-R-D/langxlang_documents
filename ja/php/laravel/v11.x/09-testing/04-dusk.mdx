...
> あなたが手動で Dusk の service provider を register する場合、それを**決して**あなたの
> production environment で register してはならない、というのもそれによって任意の
> users があなたの application で authentication できるようになる可能性があるからです。
...
> あなたがローカル開発の environment を管理するために[Laravel Sail](/ja/php/laravel/v11.x/10-packages/sail)を使用している場合は、[Dusk
> テストの設定と実行](/ja/php/laravel/v11.x/10-packages/sail#laravel-dusk)についての
> Sail のドキュメンテーションも参照してください。
...
> <LaravelNote type="warning" />
> Dusk は、`chromedriver`バイナリが実行可能であることを必要とします。 Dusk の実行に問題がある場合は、次の
> command を使用してバイナリが実行可能であることを確認する必要があります: `chmod
...
> <LaravelNote type="warning" />
> SQLite のインメモリーデータベースは、 Dusk テストを実行する際には使用できないかもしれません。ブラウザは独自の
> process 内で実行されるため、他のプロセスのインメモリーデータベースにアクセスすることはできません。
...
> <LaravelNote type="note" />
> Pest を使用している場合、基本`DuskTestCase` class や test ファイルが拡張する任意の
> class にプロパティや methods を定義する必要があります。
...
> <LaravelNote type="note" />
> [Laravel Sail](/ja/php/laravel/v11.x/10-packages/sail)を使用してローカル開発の
> environment を管理している場合は、[Dusk テストの設定と実行](/ja/php/laravel/v11.x/10-packages/sail#laravel-dusk)についての
> Sail のドキュメンテーションをご覧ください。
...
チェックボックスの入力をチェックするために、`check`method を使用することができます。他の多くの入力関連の method と同様に、CSS セレクタは必要ではありません。もし CSS セレクタのマッチが見つからない場合、Dusk は一致する`name`属性をもつチェックボックスを検索します：
...
> <LaravelNote type="warning" />
> これらのメソッドは jQuery と連携します。もしページ上で jQuery が使用できない場合、
> Dusk は自動的にページにそれを注入し、テストの期間中に利用可能となります。
...
> <LaravelNote type="note" />
> すべての修飾 keys（例：`{command}`）は`{}`文字で囲まれ、`Facebook\WebDriver\WebDriverKeys`
> class で定群されている定数と match します。これは[GitHub 上で見つけることができます](https://github.com/php-webdriver/php-webdriver/blob/master/lib/WebDriverKeys.php)。
...
> <LaravelNote type="warning" />
> ほとんどの Dusk 連続的な統合設定では、ご利用の Laravel application がビルトインの
> PHPdevelopmentserver を使用してポート 8000 で提供されることを想定しています。したがって、続行する前に、連続的な統合環境に
> `APP_URL` の環境変数 value が `http://127.0.0.1:8000`であることを確認していただく必要があります。
