...
あなたがあなたの factory ーを定義したら、`Illuminate\Database\Eloquent\Factories\HasFactory` トレイトがあなたの models に提供する static `factory` method を使ってその models のための factory ーインスタンスを生成することができます。 models の作成例をいくつか見てみましょう。まず、`make` method を使用して database に保存せずに models を作成します：
...
> [!NOTE]  
> [マスアサインメント保護](/ja/php/laravel/v11.x/08-eloquent-orm/01-eloquent#mass-assignment)は、ファクトリを使用して models を作成するときに自動的に無効になります。
...
### Has Many Relationships を直訳すると、多数の関係性を持つもしくは、多くのリレーションシップを持つとなりますが、これはプログラミングの言葉で one-to-many の意味も含まれています。ですから、日本のプログラマーの間では一対多の関係という表現が一般的です。しかしながら、もともとの英文がそのコンテクストをはっきり表していないため、それらの翻訳は推定に過ぎません。具体的な英文のコンテクストが分からない限り、正確な翻訳を提供することは難しいです
