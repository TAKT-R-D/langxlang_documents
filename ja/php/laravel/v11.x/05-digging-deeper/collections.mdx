...
> [Eloquent](/ja/php/laravel/v11.x/08-eloquent-orm/01-eloquent)クエリの結果は常に`Collection`インスタンスとして返されます。
...
> <LaravelNote type="note" />
> `collect` method は、`Enumerable`のインスタンスがあり、非遅延の collection インスタンスが必要な場合に特に便利です。
> `collect()`は`Enumerable`の契約の一部であるため、安全に使用して`Collection`インスタンスを取得できます。
...
`countBy`method は collection 内の values の出現回数を数えます。 default では、method はすべての要素の出現回数を数え、collection 内の特定の types の要素の数を数えることができます：
...
`doesntContain`の method は、アイテムの values を確認する際にルーズな比較を使用します。つまり、整数 values を持つ string は、同じ values を持つ整数と等しいとみなされます。
...
> `ensure` method は、異なる種類の elements が後から collection に追加されないことを保証しません。
...
> <LaravelNote type="warning" />
> 他のほとんどの collection メソッドとは異なり、`forget`は新たに変更されたコレクションを返すのではなく、呼び出された
> collection 自体を変更します。
...
`implode`"method は、collection のアイテムを結合します。その引数は、collection 内のアイテムの type に依存します。 collection が arrays または object を含む場合、属性の key を渡すべきです。結合したいものと、values の間に置きたい"glue"string：
...
> <LaravelNote type="warning" />
> 他のほとんどの collection メソッドと同様に、`map`は新しい collection インスタンスを返します。それは呼び出された
> collection を変更しません。元の collection を transform したい場合は、[`transform`](#method-transform)
> method を使用してください。
...
> <LaravelNote type="warning" />
> もし与えられた value が見つからない、またはコールバックが`true`を返さない場合、`skipUntil`
...
> <LaravelNote type="note" />
> ネストされた配列やオブジェクトの collection をソートする必要がある場合は、[`sortBy`](#method-sortby)
> メソッドと [`sortByDesc`](#method-sortbydesc) メソッドをご覧ください。
...
> <LaravelNote type="warning" />
> 指定された value が見つからないか、コールバックが`true`を返さない場合、`takeUntil`
...
> <LaravelNote type="warning" />
> `toArray`は、`Arrayable`のインスタンスであるコレクションのすべてのネストされた
> objects も array に変換します。 collection の下にある原始的な array を取得したい場合は、代わりに[`all`](#method-all)
> method を使用してください。
...
> <LaravelNote type="warning" />
> ほとんどの他の collection メソッドとは異なり、 `transform`は collection 自体を変更します。代わりに新しい
> collection を作成したい場合は、[`map`](#method-map) method を使用してください。
...
`whereIn`の method はアイテムの values をチェックする際にゆるい比較を使用します。つまり、整数 values を持つ string は同じ values の整数と同等とみなされます。厳密な比較を使用してフィルタリングするには、[`whereInStrict`](#method-whereinstrict)の method を使用します。
...
`whereNotIn`の method は、アイテムの values をチェックする際に ルーズな比較を使います。つまり、整数 values を持つ string は、同じ values の整数と等しいと見なされるでしょう。strict な比較を使用してフィルタリングするには、[`whereNotInStrict`](#method-wherenotinstrict) の method を使ってください。
...
> <LaravelNote type="warning" />
> collection を変更する Methods(`shift`、`pop`、`prepend` etc )は、`LazyCollection`
> class では**利用できません**。
