...
> [!WARNING] > `toArray`は、`Arrayable`のインスタンスであるコレクションのすべてのネストされた objects も array に変換します。 collection の下にある原始的な array を取得したい場合は、代わりに[`all`](#method-all) method を使用してください。
