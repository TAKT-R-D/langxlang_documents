...
> [!NOTE]  
> To learn more about working around these issues, please review the documentation regarding [queued jobs and database transactions](/docs/11.x/queues#jobs-and-database-transactions).
...
> [!NOTE]  
> `attach` method は、notification mail メッセージによって提供され、また[attach 可能なオブジェクト](/ja/php/laravel/v11.x/05-digging-deeper/mail#attachable-objects)も受け入れます。詳細については、詳細な[attach 可能な object ドキュメンテーション](/docs/11.x/mail#attachable-objects)をご覧ください。
...
もちろん、あなたはしばしばアプリケーションの users が所有する Slack ワークスペースに notifications を送りたいと思うでしょう。それを行うためには、まず user のための Slack OAuth token を取得する必要があります。ありがたいことに、[Laravel Socialite](/ja/php/laravel/v11.x/10-packages/socialite)は Slack driver を含んでおり、アプリケーションの users を Slack で簡単に認証し、[bot token を取得する](/docs/11.x/socialite#slack-bot-scopes)ことができます。
...
「 notification が送信されると、`Illuminate\Notifications\Events\NotificationSent` [event](/ja/php/laravel/v11.x/05-digging-deeper/events)が notification システムによって発行されます。これには、"notifiable"エンティティと notification インスタンスそのものが含まれます。 application 内でこの event のための[event listeners](/docs/11.x/events)を作成することもできます：」
