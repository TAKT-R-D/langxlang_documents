...
| API                                                             | 目的                                                 | 場所                                                  | 状態 Code                            |
| --------------------------------------------------------------- | ---------------------------------------------------- | ----------------------------------------------------- | ------------------------------------ |
| [`redirect`](#redirect-function)                                | ミューテーションまたはイベント後に Redirect ユーザー | Server Components 、 Server Actions 、 Route ハンドラ | 307(一時)または 303( Server Action ) |
...
`next.config.js` ファイルの `redirects` オプションは、来た requestpath を別のデスティネーション path に redirects することができます。これはページの URL 構造を変更したり、事前に知られている redirects のリストがある場合に便利です。
