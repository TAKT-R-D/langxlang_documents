...
Next.js は、オプショナルな[`src` directory](/ja/javascript/next.js/v14/02-app/01-building-your-application/07-configuring/06-src-directory)内にアプリケーションの code(`app`を含む)を保存することをサポートしています。これにより、プロジェクトの設定ファイルとアプリケーションの code が分離し、主にプロジェクトの root に live することができます。
